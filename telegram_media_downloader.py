...
def print_banner() -> None:
    ...
    title = f"{P.bold}原画质媒体下载器{P.reset}"
    features = (
        f"{P.dim}整组识别  ·  并发下载  ·  智能重试{P.reset}"
    )
...
class BatchProgress:
    ...
    def _render(self, done: int, expected: int) -> None:
        ...
        text = (
            f"{P.cyan}总进度{P.reset} {P.bright_cyan}{bar}{P.reset} "
            f"{P.bold}{percent}{P.reset} {P.gray}│{P.reset} {size_text} "
            f"{P.gray}│{P.reset} {self.items_done}/{self.total_items} 项 "
...
def parse_post_url(raw_url: str) -> TelegramPost:
    ...
    if parsed.hostname is None or parsed.hostname.lower() not in SUPPORTED_HOSTS:
        raise ValueError("只支持 t.me、telegram.me 或 telegram.dog 的帖子链接")
    ...
    if comment_values:
        ...
        if not raw_comment.isdigit() or int(raw_comment) <= 0:
            raise ValueError(f"无效的评论编号：{raw_comment}")
    ...
    if len(parts) < 2:
        raise ValueError("链接中没有找到频道名和帖子编号")

    if parts[0].lower() == "c":
        if len(parts) < 3 or not parts[1].isdigit() or not parts[2].isdigit():
            raise ValueError("无效的 Telegram 私有频道帖子链接")
        internal_id = parts[1]
    ...
    if not re.fullmatch(r"[0-9A-Za-z_]+", channel):
        raise ValueError(f"无效的频道名：{channel}")
    if not parts[1].isdigit():
        raise ValueError(f"无效的帖子编号：{parts[1]}")
...
def read_api_credentials() -> tuple[int, str]:
    ...
    if not raw_id and not api_hash:
        try:
            ...
            saved = None
            print(icon_retry(f"读取 Windows 凭据失败，将重新询问：{exc}"))
        if saved is not None:
            raw_id, api_hash = saved
            print(icon_done("已从 Windows 凭据管理器读取 API 凭据"))
    ...
    if not raw_id or not api_hash:
        ...
        print_panel(
            ...
            [
                ...
                f"{P.dim}凭据将安全保存到 Windows 凭据管理器，不会写入脚本。{P.reset}",
            ],
            P.yellow,
        )
        print()
    ...
    if not api_hash:
        api_hash = getpass.getpass(
            f"{P.bright_cyan}❯{P.reset} API_HASH（输入时不显示）: "
        ).strip()
        prompted = True
    ...
    if not re.fullmatch(r"[0-9a-fA-F]{32}", api_hash):
        raise ValueError("API_HASH 应为 32 位十六进制字符串")

    if prompted:
        try:
            save_api_credentials(api_id, api_hash)
            print(icon_done("API 凭据已安全保存，后续启动无需重复输入"))
        except OSError as exc:
            print(icon_retry(f"保存到 Windows 凭据管理器失败：{exc}"))
...
async def resolve_album_messages(
    ...
    if target is None:
        raise RuntimeError(f"找不到帖子 {message_id}，或当前账号无权访问")
...
async def resolve_comment_entity(
    ...
    if not getattr(discussion, "messages", None):
        raise RuntimeError("该频道帖子没有可访问的评论区")
    ...
    if discussion_peer is None:
        raise RuntimeError("无法识别该帖子的关联讨论组")
    ...
    try:
        ...
        raise RuntimeError("当前账号无法访问该帖子的关联讨论组")
...
async def download_one_media(
        ...
        for attempt in range(retries + 1):
            ...
            try:
                ...
                if result is None or not temporary.is_file():
                    raise IncompleteDownloadError("Telegram 未返回下载文件")

                actual_size = temporary.stat().st_size
                if actual_size <= 0:
                    raise IncompleteDownloadError("下载结果为空文件")
    ...
    raise RuntimeError(f"下载未完成：{filename}")
...
async def download_post(
    ...
    if not media_messages:
        raise RuntimeError("该帖子中没有可下载的图片或视频")
    ...
    print_panel(
        "找到完整媒体组",
        [
            f"来源　{P.bold}@{channel_name}{P.reset} · "
            f"帖子 {P.bold}#{post.message_id}{P.reset}"
            + (
                f" · 评论 {P.bold}#{post.comment_id}{P.reset}"
                if post.comment_id is not None
                else ""
            ),
            f"数量　{P.bold}{len(media_messages)} 项{P.reset}"
            f"（图片 {image_count}，视频 {video_count}）",
            f"目录　{P.bright_cyan}{output_dir}{P.reset}",
        ],
        P.bright_cyan,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="下载 Telegram 帖子整组最高质量图片和视频"
    )
    parser.add_argument(
        "url",
        nargs="?",
        help="例如 https://t.me/ExampleChannel/37?single",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="相册并发下载数，1-8（默认：3）",
    )
    parser.add_argument(
        "--retries",
        type=int,
        default=5,
        help="网络失败后的重试次数，0-10（默认：5）",
    )
    parser.add_argument(
        "--forget-credentials",
        action="store_true",
        help="删除保存在 Windows 凭据管理器中的 API_ID/API_HASH 后退出",
    )
    return parser
...
async def run_downloader(args: argparse.Namespace) -> int:
    if args.forget_credentials:
        ...
        try:
            ...
            print(icon_error(f"删除 Windows 凭据失败：{exc}"), file=sys.stderr)
            return 1
        if removed:
            print(icon_done("已删除保存的 API_ID/API_HASH"))
        else:
            print(icon_skip("没有找到已保存的 API 凭据"))
        return 0

    if not 1 <= args.jobs <= 8:
        print(icon_error("输入错误：--jobs 必须在 1 到 8 之间"), file=sys.stderr)
        return 2
    if not 0 <= args.retries <= 10:
        print(icon_error("输入错误：--retries 必须在 0 到 10 之间"), file=sys.stderr)
        return 2
    ...
    if first_url is None:
        print()
        first_url = ask("粘贴 Telegram 帖子链接（直接回车退出）:")
        if not first_url:
            return 0
    ...
    try:
        ...
        print(icon_error(f"输入错误：{exc}"), file=sys.stderr)
        return 2
    ...
    try:
        print()
        print(icon_wait("正在连接 Telegram …"))
        await client.start()
        print(icon_done(f"Telegram {P.bold}登录成功{P.reset}"))
        ...
        while current_post is not None:
            try:
                ...
                print(icon_error(f"下载失败：{exc}"), file=sys.stderr)
            ...
            print()
            next_url = ask("继续粘贴下一条帖子链接（直接回车退出）:")
            if not next_url:
                current_post = None
                continue
            try:
